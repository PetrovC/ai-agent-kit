#!/usr/bin/env bash
# pre-bash-guard.sh — Antigravity BeforeTool(run_shell_command) hook
#
# Blocks known destructive commands before they run.
# Exit code 2 = block the command and show stderr to Antigravity.
...
# own approval mode — keep `default` or `auto_edit` enabled on Antigravity. This
# guard is the cheap second layer, not the only one.
#
# Wired in .agy/settings.json:
#   { "hooks": { "BeforeTool": [
#       { "matcher": "run_shell_command",
#         "hooks": [{"type":"command","command":"bash .agy/hooks/pre-bash-guard.sh"}] } ] } }
#
# stdin schema (per https://agycli.com/docs/hooks/reference/):
#   {
