...
> Use when updating a package. One dependency at a time — do not batch unrelated updates.
> Use the dependencies skill throughout.
...
1. Read the changelog / release notes between [OLD_VERSION] and [NEW_VERSION].
   Identify:
...
7. Run the full test suite. Compare to baseline.
   If tests regressed: investigate whether it is a real behaviour change or a test that
   was testing implementation details.
...
- Test results: before vs after (X passed → Y passed).
