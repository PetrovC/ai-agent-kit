# Prompt: Tech Debt Review

> Use before a sprint or quarterly planning session to surface and prioritize technical debt.
> This is a triage run — do NOT fix anything during this pass.
...
1. Outdated dependencies
   Run the appropriate audit tool and list packages > 1 major version behind,
   especially those with known CVEs:
...
4. Missing test coverage
   Identify business logic (domain rules, use cases, state transitions) with zero test coverage.
   Provide file path and function/class name — not a coverage percentage.

5. Dead code
   Unused exported functions, commented-out blocks > 10 lines, feature flags never cleaned up,
   TODOs/FIXMEs older than 90 days.

6. Oversized units
   Files > 500 lines or functions > 50 lines where the complexity is a genuine maintenance risk
   (not cosmetic). Explain why it is a problem.
...
Sort the output: high risk + small effort items first.
