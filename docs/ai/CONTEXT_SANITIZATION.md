...
When reporting Windows hook issues, include whether `bash` resolves to Git Bash
or WSL. This repository has previously hit failures when `C:\Windows\System32`
`bash.exe` was earlier on `PATH` than Git Bash.
