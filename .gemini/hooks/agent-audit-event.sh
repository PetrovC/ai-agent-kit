#!/usr/bin/env bash
# Best-effort gemini hook integration for anonymized agent audit metadata.
# It exits zero when audit is disabled, unavailable, or unable to record so
# normal gemini behavior is unchanged.
...
project_dir = os.environ.get("gemini_PROJECT_DIR") or os.getcwd()
seed = os.environ.get("gemini_SESSION_ID") or f"{os.environ.get('USER', 'user')}:{project_dir}"
run_hash = hashlib.sha256(seed.encode("utf-8")).hexdigest()[:16]
run_id = os.environ.get("AAK_AUDIT_RUN_ID") or f"run_gemini_{time.strftime('%Y%m%d')}_{run_hash}"
project_hash = "hmac_sha256_" + hashlib.sha256(project_dir.encode("utf-8")).hexdigest()[:16]
sequence = int(time.time() * 1000)
event = {
    "schema_version": "0.1.0",
    "event_id": f"evt_gemini_{sequence}",
    "audit_run_id": run_id,
    "sequence": sequence,
